...
*/
string ArithmeticHelper::InfixToPostfix(string infix)
{
    char current = 0;
    ...
    for(int i = 0;i < infix.size(); ++i)
    {
        ...
        switch(current)
        {
            ...
            case '6':case '7':case '8':case '9':case '.':case 'P':
            case 'V':
...
/*  计算后缀表达式结果
    ...
    栈中最后的元素即为结果。
*/
double ArithmeticHelper::posfixCompute(double value,string formula)
{
...
string ArithmeticHelper::UpdateFormula(string formula)
{
    string posfixFormula;
    posfixFormula = "";
    _log.LOG_DEBUG("ArithmeticHelper 计算公式 formula = 【%s】",formula.data());
    posfixFormula = InfixToPostfix(formula);
    return posfixFormula;
}
