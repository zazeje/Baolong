...
ThreadSimulator::ThreadSimulator(DeviceInfo di): DeviceThread(di)
{
    if(_di.type == "1")
        myTcpDevice = new SimulatorTcp(atoi(_di.Port.data()),_di.Ip,_di.Name);
    _stopprocess = false;
}
...
int ThreadSimulator::getTestItemInfo()
{
    int num=0;
...
void ThreadSimulator::threadprocess()
{
    int index = 0;
    string endFlag;
    _log.LOG_INFO("ThreadSimulator 【%s】线程驱动已启动......",_di.Name.data());
    pi.init();
        ...
        for(map<string, UnitInfo>::iterator kvpu = _di.Units.begin();kvpu != _di.Units.end();kvpu++)
        {
            for(map<string, Tag>::iterator kvpt = kvpu->second.Tags.begin();kvpt != kvpu->second.Tags.end();kvpt++)
            {
                ...
                if(name == "SC" && tag.MemortValue == "1")
                {
                    ...
                    _log.LOG_DEBUG("ThreadSimulator 【%s】 检测到高度检测信号!",_di.Name.data());
                    ...
                    _log.LOG_DEBUG("ThreadSimulator 【%s】 id为【%s】 序列号为【%s】",_di.Name.data(),partNoId.data(),partSeqNo.data());

                    index = processSimulateCheck(partSeqNo,partNoId,paraVec);
    ...
    _log.LOG_INFO("ThreadSimulator 【%s】线程驱动已停止......",_di.Name.data());
}


void *ThreadSimulator::Start_Thread(void* arg)
{
    ThreadSimulator* pth = (ThreadSimulator*)arg;
    pth->threadprocess();
    return NULL;
}

bool ThreadSimulator::Start()
{
    pthread_attr_t pth_attr;
    pthread_attr_init(&pth_attr);
    pthread_attr_setdetachstate(&pth_attr,PTHREAD_CREATE_DETACHED);
    if(0 != pthread_create(&pth,&pth_attr,ThreadSimulator::Start_Thread,this))
    {
        _log.LOG_ERROR("ThreadSimulator 【%s】 高度检测设备采集线程创建失败",_di.Name.data());
        return false;
    }
    return true;
}

bool ThreadSimulator::Stop()
{
    _stopprocess = true;
    if(_di.type == "1")
    {
...
bool ThreadSimulator::initDevice()
{
//    vector<string> ret = getParameterNoInfo();
//    if(ret.size() == 23)
//    {
//        if(myTcpDevice->setPara(ret)){
//            return true;
//        }else{
//            return false;
//        }
...
/**
 * @brief ThreadSimulator::getParameterNoInfo
 * @return
 */
vector<string> ThreadSimulator::getParameterNoInfo()
{
    vector<string> ret;
    ret.clear();
    string data = "";
    ...
    {
        if(atoi((const char*)_di.parameterNo.at(i).data()) == 1)
        {
            if(_di.parameter.size() > i)
                data = _di.parameter.at(i);
            else
                _log.LOG_ERROR("ThreadSimulator getParameterNoInfo 【%s】 获取参数失败",_di.Name.data());
        }
    }
    MYSQL_RES* result;
    MYSQL_ROW row;
    ...
    string command = "";
    command.append("select * from simulatorpara where ParaIndex = ").append("'").append(data).append("';");
    _log.LOG_INFO("ThreadSimulator 【getSimulatorInfo】 command = 【%s】", command.data());
...
int ThreadSimulator::processSimulateCheck(string partSeqNo, string partNoId, vector<double> &paraVec)
{
    int index = 0;
    string value;
    if(/*!partNoId.empty() && !partSeqNo.empty()*/1)
    {
        sleep(5);
...
//        value = judgeMicrometerCheck(index,result,paraVec);
    ...
    else
    {
        ...
        _log.LOG_DEBUG("ThreadSimulator 【%s】 获取序列号、id为【空】，不进行测试，直接判定为【不良品】",_di.Name.data());
        string svalue;
    ...
    _log.LOG_DEBUG("ThreadSimulator 【%s】特征值为【%s】",_di.Name.data(),pi.testItemEigenValue.data());
    return index;
}

/**
 * @brief ThreadSimulator::CommunicateTest
 */
void ThreadSimulator::CommunicateTest()
{
    if(_di.type == "1")
    ...
    {
        _log.LOG_ERROR("ThreadSimulator 【%s】 通信检测【失败】",_di.Name.data());
        m_db.Write_TagMValue(_num + "$" + "NT",_di.Name + " 连接异常");
    }
    else
    {
        m_db.Write_TagMValue(_num + "$" + "NT","");
//                            _log.LOG_DEBUG("ThreadSimulator 【%s】 通信检测【成功】",_di.Name.data());
    }
}

string ThreadSimulator::judgeMicrometerCheck(int index,vector<double>& result,vector<double> &paraVec)
{
    string value;
    if(_di.testItemCode.size() > 0 && paraVec.size() == 2)
    {
        if(!result.empty() && result.size() == 2 )
        {
            ...
            {
                _log.LOG_DEBUG("ThreadSimulator 【%s】 minValue或者maxValue 【ERROR】",_di.Name.data());
        ...
        {
            _log.LOG_DEBUG("ThreadSimulator 【%s】 高度检测结果 【为空】",_di.Name.data());
...
//                                pi.testItemEigenValue = "NG";
        ...
        _log.LOG_DEBUG("ThreadSimulator 【%s】  高度检测结果 result = 【%s】",_di.Name.data(),value.data());
 ...
 * @param paraVec
 */
void ThreadSimulator::judgeParameterCheck(vector<double> &paraVec)
{
    if(paraVec.size() != 2)
    {
        m_db.Write_TagMValue(_di.devParaAlarm, "1");
        _log.LOG_ERROR("ThreadSimulator 【%s】 获取【加工参数】失败",_di.Name.data());
        m_db.Write_TagMValue(_di.iValue, "获取【加工参数】失败");
    }
    else
    {
        m_db.Write_TagMValue(_di.testParaAlarm, "1");
        _log.LOG_ERROR("ThreadSimulator 【%s】 获取【测试项参数】失败",_di.Name.data());
        m_db.Write_TagMValue(_di.iValue, "获取【测试项参数】失败");
    }
}

/**
 * @brief ThreadSimulator::updateJudgeResult
 * @param oktimes
 * @param index
 * @param result
 */
void ThreadSimulator::updateJudgeResult(int oktimes,int index,vector<double>& result)
{
    if(oktimes == 2)          //代表有2个高度检测设备的值，且都在规定范围以内
    {
        _log.LOG_DEBUG("ThreadSimulator 【%s】 高度检测设备的值 【正确】 最大值为【%f】 最小值为【%f】 采集值1为【%f】 采集值2为【%f】"\
...
//                                    pi.testItemEigenValue = value;
    ...
    {
        _log.LOG_ERROR("ThreadSimulator 【%s】 高度检测设备的值 【错误】 最大值为【%f】 最小值为【%f】 采集值1为【%f】 采集值2为【%f】"\
           ,_di.Name.data(),_di.maxValue.at(index),_di.minValue.at(index),result.at(0),result.at(1));
        pi.testItemJudgeResult = 0;
        m_db.Write_TagMValue(_di.JudgeResult , "0");
//                                    pi.testItemEigenValue = "NG";
...
string ThreadSimulator::getMicrometerValue(int &oktimes,int index,vector<double>& result,vector<double> &paraVec)
{
    string value;
    ...
    {
         m_db.Write_TagMValue(_di.testParaAlarm, "1");
         _log.LOG_ERROR("ThreadSimulator 【%s】 获取测试项参数【失败】 minValue=0,maxValue=0",_di.Name.data());
    }
