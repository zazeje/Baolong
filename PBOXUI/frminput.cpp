...
void frmInput::ShowPanel()
{
    ...
    ui->btnClose->setMinimumWidth(width + 30);
    ui->btnClose->setMaximumWidth(width + 30);
    ui->btnClose->setFixedHeight(50);
}
...
bool frmInput::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if (event->type() == QEvent::MouseButtonPress) {
        ...
        //中文模式下回车键frmInput::Instance()->Init("bottom", "black", 15, 10);取拼音,中文模式下当没有拼音时可以输入空格
...
//    qDebug() << "oldWidget:" << oldWidget << " nowWidget:" << nowWidget;
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
        ...
        {
            this->setGeometry(0, 0, deskWidth, frmHeight - 30);
        }
    }
}
